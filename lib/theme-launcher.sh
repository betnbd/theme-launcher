...
theme_launcher_rgb_triplet_to_signed_argb() {
  local rgb_value="$1"
  local r g b value
...
theme_launcher_write_brave_desktop_override() {
  ...
  for source_desktop in /usr/share/applications/brave-browser.desktop /usr/share/applications/com.brave.Browser.desktop; do
    ...
    awk -v extension_dir="$extension_dir" -v rgb_value="$rgb_value" '
      /^Exec=\/usr\/bin\/brave-browser-stable --incognito/ {
        sub(/^Exec=\/usr\/bin\/brave-browser-stable /, "Exec=/usr/bin/brave-browser-stable --install-autogenerated-theme=" rgb_value " --load-extension=" extension_dir " ")
        print
        next
      }
...
theme_launcher_apply_brave_profile_theme() {
  ...
  if theme_launcher_brave_running; then
    theme_launcher_warn "Brave is running; close and re-run theme-launcher apply --only chromium to update Brave's active theme immediately"
    return 0
  fi

  argb_value="$(theme_launcher_rgb_triplet_to_signed_argb "$rgb_value")"

  while IFS= read -r prefs_file; do
    [[ -f "$prefs_file" ]] || continue
    cp -a "$prefs_file" "$prefs_file.theme-launcher.bak" 2>/dev/null || true
    THEME_LAUNCHER_BRAVE_PREFS="$prefs_file" \
    THEME_LAUNCHER_BRAVE_THEME_COLOR="$argb_value" \
    python3 - <<'PY'
import json
import os
...
theme_color = int(os.environ["THEME_LAUNCHER_BRAVE_THEME_COLOR"])
data = json.loads(prefs_path.read_text())
extensions = data.setdefault("extensions", {})
theme = extensions.setdefault("theme", {})
theme.pop("pack", None)
theme["color"] = theme_color
theme["id"] = "autogenerated_theme_id"
...
theme_launcher_apply_chromium() {
  ...
  theme_launcher_write_brave_theme_extension
  theme_launcher_write_brave_desktop_override
  theme_launcher_apply_brave_profile_theme "$rgb_value"
}
