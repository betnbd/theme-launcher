...
- Brave toolbar colors through a generated local theme extension
...
theme-launcher apply retro-82 --only firefox,thunderbird,brave,codex
theme-launcher previous apply
```
...
fastfetch, bat, fzf, gtk, firefox, thunderbird, brave, vscode, codex
```

Aliases include `ff` for Firefox, `tbird` for Thunderbird, `brave-browser` for Brave, and `codex-desktop` for Codex Desktop.
...
Optional theme files include `preview.png`, `gtk.css`, `ghostty.conf`, `btop.theme`, `neovim.lua`, `vscode.json`, `icons.theme`, and a `backgrounds/` directory.
...
Theme Launcher can generate GTK, GNOME Shell, Firefox, Thunderbird, Brave, Codex Desktop, tmux, lazygit, fastfetch, bat, and fzf config from `colors.toml` during apply, so most simple themes only need palette colors plus optional app-specific overrides.
...
Brave uses its own target because managed browser policy colors are weaker and can interfere with the generated Brave theme. Theme Launcher removes its own old Brave policy file when possible, generates a local Brave theme extension from the active palette, writes Brave's active theme pack preference, and generates local desktop-entry overrides that launch Brave with the local theme extension from `~/.local/share/theme-launcher/state/brave-theme-extension`. If Brave is running during apply, Theme Launcher restarts it so the updated generated extension colors are loaded.
...
theme-launcher apply THEME --only brave
...
`theme-launcher doctor` checks dependencies, writable paths, theme asset shape, GTK bindings, stored theme references, and common GNOME integration gaps before apply time.
