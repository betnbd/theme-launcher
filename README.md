...
Brave uses a separate path because managed `BrowserThemeColor` policy blocks richer theme application. Theme Launcher removes its own old Brave policy file when possible, writes Brave's active autogenerated theme preference while Brave is closed, and generates local desktop-entry overrides that launch Brave with the current theme RGB plus the local theme extension from `~/.local/share/theme-launcher/state/brave-theme-extension`. If Brave is running during apply, close Brave and re-run:
