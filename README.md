...
Chromium and Google Chrome use managed policy files populated from each theme's `chromium.theme` RGB triplet. The browser policy directory must be writable.

Brave uses a separate path because managed `BrowserThemeColor` policy and Chromium's autogenerated theme seed both produce weaker results than the real palette. Theme Launcher removes its own old Brave policy file when possible, generates a local Chromium theme extension from the active palette, writes Brave's active theme pack preference while Brave is closed, and generates local desktop-entry overrides that launch Brave with the local theme extension from `~/.local/share/theme-launcher/state/brave-theme-extension`. If Brave is running during apply, restart Brave after apply to load the updated extension colors.

Once `THEME_LAUNCHER_ENABLE_CHROMIUM=1` is set, normal applies update browser colors; you can still force or skip this target with `--only chromium` or `--skip chromium`.
